                            ...
                            <div class="w-5/6 lg:w-1/2 mx-auto bg-gray-800 rounded-lg shadow">
                              <div class="py-4 text-white px-8 text-xl border-grey-lighter">
                                <span><strong>Actualizar</strong></span>
                                  ...
                                  <form method="POST" action="{{ route('gestionLineas.update', $gestionLinea->id) }}" enctype="multipart/form-data" class="py-4 px-8  rounded-b-xl">
                                    @csrf  
                                    @method('PUT')
                                      <div class="mb-4">
                                        ...
                                        <select name="linea_id" id="linea_id" class="appearance-none bg-gray-700 border-none text-sm rounded w-full py-2 px-3 text-gray-100">
                                          <option value="{{ $gestionLinea->linea_id }}" selected>{{ $gestionLinea->linea->linea }}</option>
                                          @foreach ($lineas as $linea)
                                            @if ($gestionLinea->linea_id != $linea->id)
                                      ...
                                      <div class="mb-4">
                                        ...
                                        <select name="personal_id" id="personal_id" class="appearance-none bg-gray-700 border-none text-sm rounded w-full py-2 px-3 text-gray-100">
                                          <option value="{{ $gestionLinea->personal_id }}" selected>{{ $gestionLinea->personal->nombre }}</option>
                                          @foreach ($personals as $personal)
                                            @if ($gestionLinea->personal_id != $personal->id)
                                      ...
                                      <div class="mb-4">
                                        ...
                                        <textarea name="observacion" id="observacion"  class="resize-none appearance-none bg-gray-700 border-none text-sm rounded w-full py-2 px-3 text-gray-100" placeholder="Observación...">{{ $gestionLinea->observacion }}</textarea>
