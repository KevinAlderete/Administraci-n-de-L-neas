                        ...
                        <div class="container mx-auto">
                          <div class="w-5/6 lg:w-1/2 mx-auto bg-gray-800 rounded-lg shadow">
                            <div class="py-4 text-white px-8 text-xl border-grey-lighter">
                              <span><strong>Crear usuario</strong></span>
                            </div>
                                <form method="POST" action="{{ route('admin.users.store') }}" class="py-4 px-8 shadow-xl rounded-b-xl">
                                  @csrf  
                                    <div class="mb-4">
                                        <label class="after:content-['*'] after:text-red-400 block text-gray-300 text-sm  mb-2" for="name">Nombre</label>
                                        <input class="appearance-none bg-gray-700 border-none text-sm rounded w-full py-2 px-3 text-gray-100" id="name" name="name" type="text" placeholder="Ingrese nombre">
                                        @error('name') <span class="text-red-400 text-sm">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="mb-4">
                                        <label class="after:content-['*'] after:text-red-400 block text-gray-300 text-sm  mb-2" for="email">Correo</label>
                                        <input class="appearance-none bg-gray-700 border-none text-sm rounded w-full py-2 px-3 text-gray-100" id="email" name="email" type="email" placeholder="Ingrese correo">
                                        @error('email') <span class="text-red-400 text-sm">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="mb-4">
                                        <label class="after:content-['*'] after:text-red-400 block text-gray-300 text-sm  mb-2" for="password">Contraseña</label>
